...
function custom_the_password_form() {
    echo do_shortcode( '[contact-form-7 id="799" title="uniekecode"]' );
  $output = '
    <form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
    <p>' . __( 'Benieuwd naar de informatie die wij voor u hebben klaargezet?<br>Log hier in met uw unieke code.' ) . '</p>
    <p><label for="' . $label . '">' . __( 'Code:' ) . ' <input name="post_password" id="' . $label . '" type="password" size="20" /></label> <input type="submit" name="Submit" value="' . esc_attr__( 'Submit' ) . '" /></p></form>
    ';
  return $output;
}
